...
Collin Cherubim
June 30, 2025
Plotting script for isofate_coupler
"""
...
if n_atmodeller != 0:
    ...
    N_tot_molecular = (
        n_H2O + n_H2 + n_O2 + n_CO + n_CO2 + n_N2 + n_S2 + n_H2O4S + n_SO2 + n_CH4
    ) * const.avogadro + NHe_a[-1]
...
if ND_a[0] != 0:
    ...
    ax1.plot(
        t_a * const.s2yr,
        binary_diffusion.H_D(T) * x1_a * (const.mu_D - const.mu_H) / H_H,
        "--",
        color="orangered",
        label="D critical",
    )  # D/H critical flux
if NO_a[0] != 0:
    ...
    ax1.plot(
        t_a * const.s2yr,
        binary_diffusion.H_O(T) * x1_a * (const.mu_O - const.mu_H) / H_H,
        "--",
        color="green",
        label="O critical",
    )  # O/H critical flux
if NC_a[0] != 0:
    ...
    ax1.plot(
        t_a * const.s2yr,
        binary_diffusion.H_C(T) * x1_a * (const.mu_C - const.mu_H) / H_H,
        "--",
        color="gold",
        label="C critical",
    )  # C/H critical flux
    ...
    ax1.plot(
        t_a * const.s2yr,
        binary_diffusion.H_N(T) * x1_a * (const.mu_N - const.mu_H) / H_H,
        "--",
        color="blue",
        label="N critical",
    )  # N/H critical flux
if NS_a[0] != 0:
    ...
    ax1.plot(
        t_a * const.s2yr,
        binary_diffusion.H_S(T) * x1_a * (const.mu_S - const.mu_H) / H_H,
        "--",
        color="purple",
        label="S critical",
    )  # S/H critical flux
...
ax5.annotate(
    f"Rp = {round(rp_a[-1] / const.Re, 2)} Re, Mp = {round(Mp / const.Me, 2)} Me, Teq = {round(T, 0)} K",
    (2e6, 5500),
    fontsize=8,
)
...
if n_atmodeller != 0:
    ax5.annotate(
        "final He concn:" + str(round(x_He_molecular, 4)) + "=" + str(round(Y[-1], 4)) + " kg/kg",
        (2e6, 4300),
        fontsize=8,
    )
    ...
    ax5.annotate(
        "final X_He (molar concn) ="
        + str(NHe_a[-1] / (NH_a[-1] + NHe_a[-1] + ND_a[-1] + NO_a[-1] + NC_a[-1])),
        (2e6, 4300),
        fontsize=8,
    )
# envelope mass
