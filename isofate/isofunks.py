...
def phiE_CP(Teq, Mp, rho_rcb, eps, Vpot, area, mu, R_env):
    ...
    phi_B = c_s*rho_rcb*np.exp(-const.G*Mp/(c_s**2*R_rcb)) # Bondi-limited escape (eq 10 Gupta & Schlichting 2020; eq 26 Ginzburg et al. 2016)
...
def Phi_minor_species(Phi_1, Phi_2, H_1, H_2, H_minor, N_values, T, minor_species_idx, light_dominant_idx, heavy_dominant_idx):
    ...
    minor_name = SYMBOLS[minor_species_idx]
...
def f_env(R_core, R_env, Rp, Mp, Teq, mu, Fp, t, thermal = True):
    ...
    t = t*const.s2yr/1e9
    if thermal == True:
        return np.real(0.05*((Rp - const.Re*(Mp/const.Me)**0.25 - R_atm(Teq, Mp, R_core, R_env, mu))*(1/(2.06*const.Re))*(Mp/const.Me)**(0.21)*(Fp/const.Fe)**(-0.044)*(t/5)**(0.18))**(1/0.59))
    elif thermal == False:
        return np.real(0.05*((Rp - const.Re*(Mp/const.Me)**0.25 - R_atm(Teq, Mp, R_core, R_env, mu))*(1/(2.06*const.Re))*(Mp/const.Me)**(0.21)*(Fp/const.Fe)**(-0.044))**(1/0.59))
...
def phi_RR(Rp, Mp, Teq, t, F0, t0=1e6, t_sat=5e8, beta=-1.23, step_fn=False, F_final=0, t_pms=2e8, pms_factor=1e2):
    ...
    c_s = np.sqrt(2*const.kb*T/const.mu_H) # sounds speed at sonic point [m/s] Murray-Clay et al 2009
