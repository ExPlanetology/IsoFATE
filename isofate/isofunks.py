...
def Phi_O_Z90(Phi_H, Phi_He, H_H, H_O, H_He, N_H, N_He, N_D, N_O, N_C, N_N, N_S, T):
    ...
    b_He_O = (
        2.61e19 * T**0.75
    )  # [molecules/m/s] approximated from b_H_O using Genda/Ikoma 2008 prescription (Appendix C)
...
def Phi_C_Z90(Phi_H, Phi_He, H_H, H_C, H_He, N_H, N_He, N_D, N_O, N_C, N_N, N_S, T):
    ...
    b_He_C = (
        2.64e19 * T**0.75
    )  # [molecules/m/s] approximated from b_He_O using Genda/Ikoma 2008 prescription (Appendix C)
...
def make_atmosphere_descent(Tem, mu, rplanet, Mc, gamma, output_mode):
    ...
    for i in range(nr - 1, 0, -1):
        ...
        T[i - 1] = (
            Tem / (pem / p[i - 1]) ** K
        )  # dry adiabat assumption; K = R/cp = (gamma - 1)/gamma
...
def make_atmosphere_descent_jax(Tem, mu, rplanet, Mc, gamma, output_mode):
    """JAX/diffrax-jittable equivalent of `make_atmosphere_descent`.
